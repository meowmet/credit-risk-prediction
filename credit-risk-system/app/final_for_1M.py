...
# 1M LendingClub-style dataset
FE_V1_PATH = Path("credit_1M_fe_v1_fixed.csv")

# Outputs for multi-version FE + RF (Day 5 style)
OUTPUT_DIR_MULTI = Path("outputs_1M_day5")
OUTPUT_DIR_MULTI.mkdir(parents=True, exist_ok=True)

# Outputs for advanced RF + feature selection
OUTPUT_DIR_RF_ONLY = Path("outputs_1M_rf_only")
OUTPUT_DIR_RF_ONLY.mkdir(parents=True, exist_ok=True)

# IMPORTANT: adjust if your label column is named differently
TARGET_COL = "target_default_1M"
RANDOM_STATE = 42

# Training caps (to keep runtime/memory reasonable on 1M rows)
MAX_TRAIN_ROWS_MULTI = 600_000      # for multi-version RF
MAX_TRAIN_ROWS_ADV = 600_000        # for advanced RF

# SHAP config (reduced for 1M)
SHAP_SAMPLE_SIZE_MULTI = 1_000
RUN_SHAP_ADV = True
SHAP_SAMPLE_SIZE_ADV = 1_000
...
def build_fe_v2(df_v1: pd.DataFrame) -> pd.DataFrame:
    ...
    Assumes LendingClub-style columns:
      loan_amnt, annual_inc, revol_bal, total_rev_hi_lim,
      fico_range_low, fico_range_high, open_acc, total_acc, delinq_2yrs,
      term, int_rate
    """
...
def build_fe_v3(df_v2: pd.DataFrame) -> pd.DataFrame:
    ...
    FE v3: start from FE v2, add coarse bins (risk bands)
      - fico_mean_v2 bins
      - dti bins
      - loan_to_income bins (if available)
    """
    ...
    # loan_to_income bands (if we have loan_to_income from advanced FE, but keep here as optional)
...
def train_rf_for_version(
    ...
    model_path = OUTPUT_DIR_MULTI / f"rf_1M_{fe_name}.pkl"
...
def run_multi_version_block() -> None:
    ...
    """
    print("\n========== BLOCK 1: Multi-version FE + RF + SHAP (1M) ==========\n")
    ...
    cmp_path = OUTPUT_DIR_MULTI / "rf_1M_comparison.csv"
    cmp_df.to_csv(cmp_path, index=False)
    print(f"\n[RESULT] Saved RF comparison -> {cmp_path}")
    print(cmp_df)

    # Univariate AUC on v3
    auc_path = OUTPUT_DIR_MULTI / "univariate_auc_1M_v3.csv"
    compute_univariate_auc(df_v3, TARGET_COL, auc_path)

    # SHAP on best FE version
    ...
    print("\n[BLOCK 1 DONE] Multi-version FE + RF + SHAP (1M) completed.\n")
...
def build_advanced_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Advanced FE designed for RandomForest (no leakage, LendingClub-style):
...
def run_advanced_rf_block() -> None:
    ...
    """
    print("\n========== BLOCK 2: Advanced RF + Feature Selection + SHAP (1M) ==========\n")
    ...
    model_path = OUTPUT_DIR_RF_ONLY / "rf_1M_advanced.pkl"
    joblib.dump(rf, model_path)
    ...
    metrics_path = OUTPUT_DIR_RF_ONLY / "rf_1M_metrics.csv"
    metrics_df.to_csv(metrics_path, index=False)
    print(f"\n[SAVE_ADV] Metrics -> {metrics_path}")

    # Save best threshold
    with open(OUTPUT_DIR_RF_ONLY / "rf_1M_best_threshold.txt", "w") as f:
        f.write(f"best_thr_val_f1={best_thr:.4f}\n")
        f.write(f"best_f1_val={best_f1:.4f}\n")

    # 8) SHAP analysis on validation
    run_shap_analysis_advanced(rf, X_val, model_name="rf_1M_advanced")

    print("\n[BLOCK 2 DONE] Advanced RF + feature selection + SHAP (1M) completed.\n")
...
def run_pipeline():
    ...
    print("\n[ALL DONE] final_for_1M pipeline finished.")
